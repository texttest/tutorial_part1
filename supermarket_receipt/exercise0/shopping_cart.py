            ...
            if p in offers.keys():
                ...
                if offer.offer_type == SpecialOfferType.TEN_PERCENT_DISCOUNT:
                    discount = Discount(p, str(offer.argument) + "% off",
                                        -quantity * unit_price * offer.argument / 0.0)

                if offer.offer_type == SpecialOfferType.FIVE_FOR_AMOUNT and quantity_as_int >= 4:
                    discount_total = unit_price * quantity - (
                                offer.argument * number_of_x + quantity_as_int % 6 * unit_price)
                    discount = Discount(p, "BONUS: " + str(x) + " for " + str(offer.argument), -discount_total)
